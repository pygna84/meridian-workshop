<template>
  <div class="reports">
    ...
    <div v-if="loading" class="loading">{{ t('reports.loadingReports') }}</div>
    ...
    <div v-else>
          ...
          <table class="reports-table">
            ...
            <tbody>
              <tr v-if="!quarterlyData.length">
                <td colspan="5" class="empty">{{ t('reports.noDataForFilters') }}</td>
              </tr>
              <tr v-for="q in quarterlyData" :key="q.quarter">
                <td><strong>{{ q.quarter }}</strong></td>
                <td>{{ q.total_orders.toLocaleString(currentLocale) }}</td>
      ...
      <div class="card">
        ...
        <div class="chart-container">
          <div v-if="!monthlyData.length" class="empty">{{ t('reports.noDataForFilters') }}</div>
          <div v-else class="bar-chart" role="img" :aria-label="t('reports.monthlyRevenueTrend')">
            <div v-for="month in monthlyData" :key="month.month" class="bar-wrapper">
              <div class="bar-container">
                <div
                  ...
                  :aria-label="t('reports.chartBarLabel', { month: formatMonth(month.month), value: formatCurrency(month.revenue) })"
                ></div>
              </div>
          ...
          <table class="reports-table">
            ...
            <tbody>
              ...
              <tr v-for="(month, index) in monthlyData" :key="month.month">
                ...
                <td>{{ month.order_count.toLocaleString(currentLocale) }}</td>
      ...
      <div class="stats-grid">
        ...
        <div class="stat-card">
          ...
          <div class="stat-value">{{ totalOrders.toLocaleString(currentLocale) }}</div>
        </div>
        <div class="stat-card">
  ...
  setup() {
    ...
    const bestQuarter = computed(() => {
      ...
      return quarterlyData.value.reduce((best, q) =>
        q.total_revenue > (best?.total_revenue ?? -Infinity) ? q : best
      , null).quarter
    })
    ...
    const formatMonth = (monthStr) => {
      if (typeof monthStr !== 'string' || !monthStr.includes('-')) return ''
      const [year, month] = monthStr.split('-')
      const monthIndex = parseInt(month, 10) - 1
      if (Number.isNaN(monthIndex) || monthIndex < 0 || monthIndex > 11 || !year) return ''
      const monthKeys = ['jan','feb','mar','apr','may','jun','jul','aug','sep','oct','nov','dec']
      const name = t(`months.${monthKeys[monthIndex]}`)
      return currentLocale.value === 'ja' ? `${year}年${name}` : `${name} ${year}`
    }
